import { getInfiniteTVSeries } from "@/app/services/tvSeries/getInfiniteTVSeries";
import { useInfiniteQuery } from "@tanstack/react-query";

// with useInfiniteQuery that react-query provides we can provide infinite scroll
export function useInfiniteTVSeries() {
  const {
    ...
    queryKey: ["inf-tvSeries"],
    queryFn: getInfiniteTVSeries,
    getNextPageParam: (lastPage, allPages) => {
      // if the lasPage has length(exist) then return nextPage othewise retunr undfind
